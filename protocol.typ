...
#let encode-Params(value) = {
  encode-list(value.at("initial_guess"), encode-float) + encode-float(value.at("tolerance")) + encode-int(value.at("max_iterations"))
}
#let decode-Result(bytes) = {
  let offset = 0
  let (f_minimum_point, size) = decode-list(bytes.slice(offset, bytes.len()), decode-float)
  offset += size
  let (f_minimum_value, size) = decode-float(bytes.slice(offset, bytes.len()))
  offset += size
  let (f_iterations, size) = decode-int(bytes.slice(offset, bytes.len()))
  offset += size
  let (f_success, size) = decode-bool(bytes.slice(offset, bytes.len()))
  offset += size
  ((
    minimum_point: f_minimum_point,
    minimum_value: f_minimum_value,
    iterations: f_iterations,
    success: f_success,
  ), offset)
}
#let encode-Request(value) = {
  encode-Params(value.at("params"))
}
#let decode-Response(bytes) = {
  let offset = 0
  let (f_result, size) = decode-Result(bytes.slice(offset, bytes.len()))
  offset += size
  ((
    result: f_result,
  ), offset)
